...
@Component({
  ...
  styleUrl: './modifier-utilisateur.component.scss',
})
    ...
    this.route.params.subscribe((parametres) => {
      ...
      if (parametres['id'] && !isNaN(parametres['id'])) {
        ...
        if (jwt) {
          this.http
            .get(
              'http://backendfilrouge/get-user.php?id=' +
                parametres['id'],
  ...
  onAjoutUtilisateur() {
    if (this.formulaire.valid) {
      const url = this.userId
        ? 'http://backendfilrouge/edit-user.php?id=' +
          this.userId
        : 'http://backendfilrouge/ajout-utilisateur.php';
